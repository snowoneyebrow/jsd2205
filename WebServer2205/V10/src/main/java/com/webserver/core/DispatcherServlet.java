...
public class DispatcherServlet {
    ...
    public void service(HttpServletRequest request, HttpServletResponse response) {
        ...
        if (file.isFile()) { //isFile()会先判断是否存在并且是个文件
            ...
            response.addHeader("Content-Length",String.valueOf(file.length())); //length是long，或file.length()+""
        } else {
            response.setStatusCode(404);
            response.setStatusReason("NotFound");
            response.setContentFile(new File(staticDir, "/root/404.html"));response.addHeader("Content-Type","text/html");
